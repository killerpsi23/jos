...
sys_exofork(void)
{
	...
	if (ret != 0)
		return ret;
	t = child->env_thd_head;
	t->thd_tf = curthd->thd_tf;
	t->thd_tf.tf_regs.reg_eax = 0;
...
sys_env_set_status(envid_t envid, int status)
{
	...
	// LAB 4: Your code here.
	struct Env *tar;
	int ret = envid2env(envid, &tar, 1);
	if (ret != 0)
		return ret;
	if (status != ENV_RUNNABLE && status != ENV_NOT_RUNNABLE)
		return -E_INVAL;
	tar -> env_status = status;
	return 0;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (ret != 0)
		return ret;
	if ((unsigned)va % PGSIZE != 0 || (unsigned)va >= UTOP)
		return -E_INVAL;
	int must_perm = PTE_U | PTE_P;
	if ((perm & must_perm) != must_perm)
		return -E_INVAL;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((unsigned)srcva % PGSIZE != 0 || (unsigned)srcva >= UTOP || (unsigned)dstva % PGSIZE != 0 || (unsigned)dstva >= UTOP)
		return -E_INVAL;
	pte_t *tar;
	struct PageInfo *pg = page_lookup(src->env_pgdir, srcva, &tar);
	if (pg == NULL)
		return -E_INVAL;
	int must_perm = PTE_U | PTE_P;
	if ((perm & must_perm) != must_perm)
	...
	if ((perm & must_perm) != perm)
		return -E_INVAL;
	// Q: How about COW pages?
	if ((perm & PTE_W) && !((*tar) & PTE_W))
		return -E_INVAL;

	ret = page_insert(dst->env_pgdir, pg, dstva, perm);
	if (ret < 0)
		return -E_NO_MEM;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((unsigned)srcva < UTOP && (unsigned)e->env_ipc_dstva < UTOP)
	{
		pte_t *tar;
		struct PageInfo *pg = page_lookup(curenv->env_pgdir, srcva, &tar);
		if (pg == NULL)
			return -E_INVAL;
	...
	e->env_ipc_thd->thd_status = ENV_RUNNABLE;
	e->env_ipc_thd->thd_tf.tf_regs.reg_eax = 0;
...
sys_ipc_recv(void *dstva)
{
	...
	if (curenv->env_ipc_recving)
		return -E_IPC_RECVING;

	curenv->env_ipc_dstva = dstva;
	curenv->env_ipc_recving = 1;
	curenv->env_ipc_thd = curthd;

	curthd->thd_status = ENV_NOT_RUNNABLE;
	sched_yield();
}
